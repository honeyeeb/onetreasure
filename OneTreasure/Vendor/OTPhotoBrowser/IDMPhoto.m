...
// Properties
@synthesize underlyingImage = _underlyingImage, photoURL = _photoURL, caption = _caption;
...
- (id)initWithURL:(NSURL *)url {
	if ((self = [super init])) {
		_photoURL = [url copy];
        _originPhotoURL = [NSURL URLWithString:[NSString stringWithFormat:@"%@/0",[[url absoluteString] stringByDeletingLastPathComponent]]];
	}
	return self;
}
...
- (void)loadOriginUnderlyingImageAndNotify {
    ...
    if (self.underlyingImage) {
        ...
        if (_photoPath) {
            // Load async from file
            [self performSelectorInBackground:@selector(loadImageFromFileAsync) withObject:nil];
        } else if (_photoURL && _originPhotoURL) {
            
            //该实现被我替换成了SDWebImage
            SDWebImageManager *manager = [SDWebImageManager sharedManager];
            [manager downloadImageWithURL:_originPhotoURL
                                  options:0
                                 progress:^(NSInteger receivedSize, NSInteger expectedSize) {
                                     // progression tracking code
...
- (void)unloadUnderlyingImage {
	...
	if (self.underlyingImage && (_photoPath || _photoURL || _originPhotoURL)) {
		self.underlyingImage = nil;
	}
}
