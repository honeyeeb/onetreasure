...
@interface HBKeyValueStore ()

@property (strong, nonatomic) FMDatabaseQueue * dbQueue;

@end

@implementation HBKeyValueStore
...
- (void)createTableWithName:(NSString *)tableName {
    if ([HBKeyValueStore checkTableName:tableName] == NO) {
        return;
    }
...
- (id)getObjectById:(NSString *)objectId fromTable:(NSString *)tableName {
    HBKeyValueItem * item = [self getHBKeyValueItemById:objectId fromTable:tableName];
    if (item) {
...
- (HBKeyValueItem *)getHBKeyValueItemById:(NSString *)objectId fromTable:(NSString *)tableName {
    ...
    if (json) {
        ...
        HBKeyValueItem * item = [[HBKeyValueItem alloc] init];
...
- (NSArray *)getAllItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (HBKeyValueItem * item in result) {
        error = nil;
